function name = SimRobotMill_KUKAODE_Tool_function(Spindle_speed,Axial_depth)
...
slice_dphase = 2*pi/360/2; % [rad] phase difference between each 2 slices.
slice_n = ceil(Ap*tand(Helix)/(R*slice_dphase)); % num of slices
% * Derived Parameters
...
for i = 1:1:sim_n_dt_total
    ...
    if i <= sim_n_dt_start 
        ...
        while 1
            ...
            real_2_TCP_mm = real_2_TCP_mm.* [1 -1]; %%%%%%%%%%%%%%%%%%%%%%%⚪multiple wrongs！
...
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
% Result storage
% name = ['F:\Result\SimRobotMill_KUKACoupleODE_Tool_',num2str(N),'rpm_',num2str(Ap),'mm.mat'];
name = ['D:\Reuslt\MillingCuttingForceSimulation\SimTool 20230329 formal\SimRobotMill_KUKACoupleODE_Tool_',num2str(N),'rpm_',num2str(Ap),'mm.mat'];
save(name)
